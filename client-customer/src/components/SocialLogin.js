...
const GoogleLoginButton = ({ onSuccess, onFailure, text = "Continue with Google" }) => {
  const handleGoogleLogin = () => {
    ...
    // For demo purposes, we'll call onSuccess after a short delay
    setTimeout(() => {
      if (onSuccess) {
...
const FacebookLoginButton = ({ onSuccess, onFailure, text = "Continue with Facebook" }) => {
  const handleFacebookLogin = () => {
    // In real implementation, this would integrate with Facebook SDK
    // For now, we'll simulate the response
    const mockFacebookResponse = {
      ...
      picture: {
        data: {
          url: 'https://graph.facebook.com/987654321/picture'
        }
      }
    };
    ...
    setTimeout(() => {
      if (onSuccess) {
        onSuccess(mockFacebookResponse);
      }
    }, 1000);
  };
...
const SocialLogin = () => {
  ...
  const handleSocialLogin = async (provider) => {
    try {
      ...
      // Store user data (in real app, you'd integrate with your auth system)
      localStorage.setItem('socialUser', JSON.stringify(mockUser));
      alert(`${t('socialLoginSuccess')} ${provider}!`);
      
      // Redirect to home page
      window.location.href = '/home';
    } catch (error) {
      alert(`${t('socialLoginError')} ${provider}. ${t('pleaseRetry')}`);
    }
  };
